...
describe("panel", function()
  ...
  it("toggles case on/off (off => ignore, on => sensitive)", function()
    local dir = make_tree({ ["a.ts"] = "x\n" })
    local p = panel_mod.open({ cwd = dir, scope = "path", scope_paths = { dir } })
    assert.equals(false, p.toggles.case) -- default OFF
    assert.equals("ignore", p:gather_query().case)
    p:toggle("case")
    assert.equals(true, p.toggles.case)
    assert.equals("sensitive", p:gather_query().case)
    p:toggle("case")
    assert.equals(false, p.toggles.case)
    p:close()
  end)

  it("toggles replace mode on and off with the replace key", function()
    local dir = make_tree({ ["a.ts"] = "handleRequest\n" })
    local p = panel_mod.open({ cwd = dir, scope = "path", scope_paths = { dir } })
    p:_set_values({ search = "handleRequest" })
    vim.api.nvim_set_current_win(p.form_win)
    local key = vim.api.nvim_replace_termcodes("<C-d>", true, false, true)
    vim.api.nvim_feedkeys(key, "x", false)
    assert.equals("preview", p.mode)
    vim.api.nvim_feedkeys(key, "x", false)
    assert.equals("search", p.mode)
    p:close()
  end)
